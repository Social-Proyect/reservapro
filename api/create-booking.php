...
try {
    ...
    if (!$empleado_id) {
        ...
        $stmt = $db->prepare("
            ...
            AND NOT EXISTS (
                ...
                AND (
                    (c.fecha_hora < ? AND datetime(c.fecha_hora, '+' || c.duracion_minutos || ' minutes') > ?)
                    OR (c.fecha_hora >= ? AND c.fecha_hora < datetime(?, '+' || ? || ' minutes'))
                )
            )
            LIMIT 1
        ");
    ...
    $stmt = $db->prepare("
        ...
        AND (
            (fecha_hora < ? AND datetime(fecha_hora, '+' || duracion_minutos || ' minutes') > ?)
            OR (fecha_hora >= ? AND fecha_hora < datetime(?, '+' || ? || ' minutes'))
        )
    ");
